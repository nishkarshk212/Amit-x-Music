...
from Amit import app, config, db, lang
from Amit.helpers import buttons, utils
...
async def _left_member(_, message: types.Message):
    """Send log when bot leaves a group"""
    if not message.left_chat_member:
        return
    
    if message.left_chat_member.id == app.id:
        try:
            chat_name = message.chat.title if message.chat.title else "Unknown Chat"
            log_text = f"Bot LEFT from group: {chat_name} (ID: {message.chat.id})"
