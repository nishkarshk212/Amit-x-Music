...
from Amit import config, logger, userbot
...
class MongoDB:
    ...
    async def get_admins(self, chat_id: int, reload: bool = False) -> list[int]:
        from Amit.helpers._admins import reload_admins
    ...
    # SONG METHODS
    async def add_song(self, video_id: str, chat_id: int = None) -> None:
        """Store a song ID in the database"""
        await self.songsdb.update_one(
            {"_id": video_id},
            {"$set": {"video_id": video_id, "last_played": time()}},
            upsert=True
        )
        if chat_id:
            await self.songsdb.update_one(
                {"_id": video_id},
                {"$addToSet": {"played_in": chat_id}}
            )
    
    async def get_song(self, video_id: str) -> dict | None:
        """Get a stored song by ID"""
        return await self.songsdb.find_one({"_id": video_id})
    
    async def is_song_stored(self, video_id: str) -> bool:
        """Check if a song is already stored"""
        return await self.songsdb.count_documents({"_id": video_id}) > 0
